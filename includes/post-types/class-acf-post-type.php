...
if ( ! class_exists( 'ACF_Post_Type' ) ) {
	class ACF_Post_Type extends ACF_Internal_Post_Type {
		...
		public function ajax_validate_values() {
			...
			if ( preg_match( '/^[a-z_-]*$/', $post_type_key ) !== 1 ) {
				$valid = false;
				acf_add_internal_post_type_validation_error( 'post_type', __( 'The post type key must only contain lowercase letters, underscores or dashes.', 'acf' ) );
			}

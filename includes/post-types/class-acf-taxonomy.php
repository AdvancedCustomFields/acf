...
if ( ! class_exists( 'ACF_Taxonomy' ) ) {
	class ACF_Taxonomy extends ACF_Internal_Post_Type {
		...
		public function ajax_validate_values() {
			...
			if ( preg_match( '/^[a-z_-]*$/', $taxonomy_key ) !== 1 ) {
				$valid = false;
				acf_add_internal_post_type_validation_error( 'taxonomy', __( 'The taxonomy key must only contain lowercase letters, underscores or dashes.', 'acf' ) );
			}
			...
			$valid = apply_filters( "acf/{$this->hook_name}/ajax_validate_values", $valid, $_POST ); // phpcs:ignore WordPress.Security -- Raw input send to hook for validation.
		...
		public function get_taxonomy_args( $post ) {
			...
			// WordPress defaults to false.
			if ( isset( $rewrite['hierarchical'] ) && $rewrite['hierarchical'] && $rewrite_enabled ) {
		...
		public function import_cptui_taxonomy( $args ) {
			...
			if ( isset( $args['rewrite_hierarchical'] ) ) {
				$acf_args['rewrite']['hierarchical'] = $args['rewrite_hierarchical'] === '1';

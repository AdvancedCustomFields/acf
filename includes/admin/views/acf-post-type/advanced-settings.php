...
foreach ( acf_get_combined_post_type_settings_tabs() as $tab_key => $tab_label ) {
	...
	switch ( $tab_key ) {
		case 'general':
			...
			$available_supports = apply_filters( 'acf/post_type/available_supports', $available_supports, $acf_post_type );
			...
			acf_render_field_wrap(
				array(
					...
					'prefix'                    => 'acf_post_type',
					'value'                     => array_unique( array_filter( $acf_post_type['supports'] ) ),
		...
		case 'visibility':
			...
			acf_render_field_wrap(
				array(
					...
					'instructions' => __( 'By default the post type will get a new top level item in the admin menu. If an existing top level item is supplied here, the post type will be added as submenu item under it.', 'acf' ),
					'conditions'   => array(
						'field'    => 'show_in_menu',
						'operator' => '==',
						'value'    => 1,
			...
			$acf_show_metabox_cb_field = ! empty( $acf_post_type['register_meta_box_cb'] );
			if ( $acf_show_metabox_cb_field ) {
				acf_render_field_wrap( array( 'type' => 'seperator' ) );
				...
				acf_render_field_wrap(
					array(
						...
						'instructions' => __( 'A PHP function name to be called to handle the content of a meta box on your post type.', 'acf' ),
					),
					'div',
					'field'
				);
		...
		case 'urls':
			acf_render_field_wrap(
				array(
					...
					'value'        => $acf_post_type['rewrite']['permalink_rewrite'],
